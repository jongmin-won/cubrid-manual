...
.. note::
    ...
    11.2 버전 전까지는 테이블, 트리거, 시리얼 이름의 최대 길이가 254바이트였다. 11.2 버전부터 테이블, 트리거, 시리얼 및 Java 저장 프로시저 이름에는 스키마 이름이 포함된다. 따라서 식별자 이름은 소유자 이름의 길이인 31바이트와 구분자(.)의 길이인 1바이트를 제외하고 222바이트를 넘을 수 없다.
